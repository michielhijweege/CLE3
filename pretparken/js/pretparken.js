...
const apiUrl = 'json/pretparken.json';
const apidetailUrl = 'json/pretparkendetails.json';
let section;
let list;
let saveitems = [];
let detailpage;
...
function parkenSuccesHandler(data){
    console.log(data)
    for (let i = 0; i < data.pretparken.length; i++){
        const newpretparkdiv = document.createElement('section')
        newpretparkdiv.classList.add("content-box")
        newpretparkdiv.classList.add(data.pretparken[i].title)
        newpretparkdiv.id = data.pretparken[i].title

        let img = document.createElement("img")
        img.src = data.pretparken[i].img
        img.classList.add("logo-pretpark")
        newpretparkdiv.appendChild(img)

        let midcontent = document.createElement('section')
        midcontent.classList.add('midcontent')

        let h2 = document.createElement("h2")
        h2.innerHTML = data.pretparken[i].title

        newpretparkdiv.appendChild(img)
        newpretparkdiv.appendChild(midcontent)
...
function detailSuccesHandler(data){
    for (let i = 0; i < data.pretparkdetail.length; i++) {
        let link = document.createElement("a")
        link.innerHTML = data.pretparkdetail[i].linktext
        link.href = data.pretparkdetail[i].link
